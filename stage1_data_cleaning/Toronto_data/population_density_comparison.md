...
若采纳本报告建议，队友在最终导出前需要在QGIS中将人口密度图层的分级字段改为`population_density_km2`，仍采用5级Natural Breaks（Jenks）。按当前数据重新计算的预期分级为：
...
这些阈值由普通Python对当前585个官方密度值执行Jenks计算得到，需在QGIS重新分类后由人工确认QGIS显示阈值与数量，再更新布局副标题并正式导出。
